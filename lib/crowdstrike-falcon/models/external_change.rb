  ...
  class ExternalChange
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Falcon::ExternalChange` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Falcon::ExternalChange`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [action_timestamp, action_type, aid, cid, command_line, diff, entity_path, entity_path_new, entity_type, host, id, ingestion_timestamp, is_suppressed, platform_name, policy, process_id, process_image_file_name, severity, user_id, user_name].hash
    end
