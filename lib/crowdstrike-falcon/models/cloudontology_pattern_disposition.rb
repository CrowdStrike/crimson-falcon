...
module Falcon
  class CloudontologyPatternDisposition
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Falcon::CloudontologyPatternDisposition` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Falcon::CloudontologyPatternDisposition`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def hash
      [bootup_safeguard_enabled, critical_process_disabled, detect, fs_operation_blocked, inddet_mask, indicator, kill_parent, kill_process, kill_subprocess, operation_blocked, policy_disabled, process_blocked, quarantine_file, quarantine_machine, registry_operation_blocked, rooting, sensor_only].hash
    end
